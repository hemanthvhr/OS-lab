...
extern int create_myfs(int myfs_size)
{
	...
	for(int i=0;i<8;i++)
		inodelist[0]->data_pntrs[i]=0;
	inodelist[0]->indirect_pntr=0;
	inodelist[0]->dbl_ind_pntr=0;
	return 0;
}
...
{
	for(int i=1;i<super_block->max_no_dblocks;i++)
	{
		if(testbit(A,i)==0)
		{
			setbit(A,i);
...
int update_myfs(int filesize,char* dest,FILE* fp)
{
	...
	int j=0,k=0;
	while(inodelist[current_directory]->data_pntrs[j]!=0)
	{
		char z[30];
		memcpy(z,data+256*(inodelist[current_directory]->data_pntrs[j])+32*k,30);
		if(strcmp(z,"")==0)
			break;
		else
		{
	...
	if(j!=8)
	{
		if(inodelist[current_directory]->data_pntrs[j]==0)
		{
			int freedblock = getandsetfreeblock(super_block->db_bitmap);
			if(freedblock==-1)
				return -1;
			inodelist[current_directory]->data_pntrs[j] = freedblock;
			memset(data+256*inodelist[current_directory]->data_pntrs[j],0,256);
			memcpy(data+256*inodelist[current_directory]->data_pntrs[j],dest,strlen(dest)+1);
			//printf("%d\n",inodelist[current_directory]->data_pntrs[j]);
			short int *num = (short int *)(data+256*inodelist[current_directory]->data_pntrs[j]+30);
			*num = currfileinode;
		}
		else
		{
			...
			//printf("%s\n",dest);
			memcpy(data+256*inodelist[current_directory]->data_pntrs[j]+k*32,dest,strlen(dest)+1);
			short int *num = (short int *)(data+256*inodelist[current_directory]->data_pntrs[j]+k*32+30);
			*num = currfileinode;
		}
	}
	else
	...
	while(rem_size > 0)
	{
		...
		if(freedblock==-1)
			return -1;
		inodelist[currfileinode]->data_pntrs[i] = freedblock;
		fread(data+256*inodelist[currfileinode]->data_pntrs[i],256,1,fp);
		//bzero(buff,256);
	...
	if(rem_size > 0)
	{
		...
		if(indirectpointer==-1)
			return -1;
		inodelist[currfileinode]->indirect_pntr = indirectpointer;
	}
	i=0;
	while(rem_size > 0)
	{
		...
		if(freedblock==-1)
			return -1;
		unsigned int *x = (unsigned int *)(data+256*inodelist[currfileinode]->indirect_pntr + i);
		*x = freedblock;
	...
	if(rem_size > 0)
	{
		...
		if(doubleindptr==-1)
			return -1;
		inodelist[currfileinode]->dbl_ind_pntr = doubleindptr;
	}
	j=0;
	while(rem_size > 0)
	{
		if(rem_size > 0)
		{
			...
			if(indirectpointer==-1)
				return -1;
			inodelist[currfileinode]->indirect_pntr = indirectpointer;
			unsigned int *x = (unsigned int *)(data+256*inodelist[currfileinode]->dbl_ind_pntr + j);
			*x = indirectpointer;
		}
		int i=0;
		while(rem_size > 0)
...
extern int searchinodewithfilename(char* filename)
{
	...
	{
		char tempfile[30];
		memcpy(tempfile,data+256*inodelist[current_directory]->data_pntrs[i]+32*j,30);
		tempfile[strlen(tempfile)]='\0';
		int inode=*(data+256*inodelist[current_directory]->data_pntrs[i]+32*j+30);
...
extern int ls_myfs()
{
	...
	{
		char filename[30];
		memcpy(filename,data+256*inodelist[current_directory]->data_pntrs[i]+32*j,30);
		j++;
		if(j==8)
		{
			i++;
...
extern int showfile_myfs(char* filename)
{
	...
	{
		bzero(buff,256);
		memcpy(buff,data+256*inodelist[currfileinode]->data_pntrs[i],256);
		printf("%s",buff);
		i++;
		rem_size-=256;
		if(i==8)
	...
	while(rem_size > 0)
	{
		int datablock = *(data+256*inodelist[currfileinode]->indirect_pntr + i);
		bzero(buff,256);
	...
	while(rem_size > 0)
	{
		int indirectblock = *(data+256*inodelist[currfileinode]->dbl_ind_pntr + j);
		int i=0;
		while(rem_size > 0)
		{
...
int srch_del_myfs(char* filename)
{
	...
	while(temp<inodelist[current_directory]->file_size)
	{
		...
		if(!strcmp(filename,tempfile))
		{
			...
			inodelist[current_directory]->lastmodified = time(NULL);
			inodelist[current_directory]->lastaccessed = time(NULL);
			memset(data+256*inodelist[current_directory]->data_pntrs[i]+32*j,0,32);
			/*if(j==0)
			{
...
extern int rm_myfs(char* filename)
{
	...
	else
	{
		...
		{
			clearbit(super_block->db_bitmap,inodelist[file_inode]->data_pntrs[i]);
			super_block->dblocks_used--;
			temp+=256;
			i++;
			if(i==8)
		...
		{
			i=0;
			while(*(data+256*inodelist[file_inode]->indirect_pntr+i)!=0)
			{
				clearbit(super_block->db_bitmap,*(data+256*inodelist[file_inode]->indirect_pntr+i));
				super_block->dblocks_used--;
				temp+=256;
				i++;
				if(i==64)
			...
			clearbit(super_block->db_bitmap,inodelist[file_inode]->indirect_pntr);
			super_block->dblocks_used--;
		}
		if((i==64)&&(temp<inodelist[file_inode]->file_size))
		{
			...
			while(temp<inodelist[file_inode]->file_size)
			{
				int ind_block = *(data+256*inodelist[file_inode]->dbl_ind_pntr+i);
				int j=0;
				while(temp < inodelist[file_inode]->file_size)
				{
...
extern int mkdir_myfs(char* dirname)
{
	...
	if(j!=8)
	{
		if(inodelist[current_directory]->data_pntrs[j]==0)
		{
			...
			inodelist[current_directory]->data_pntrs[j] = freedblock;
			memset(data+256*inodelist[current_directory]->data_pntrs[j],0,256);
			memcpy(data+256*inodelist[current_directory]->data_pntrs[j],dirname,strlen(dirname)+1);
			//printf("%d\n",inodelist[current_directory]->data_pntrs[j]);
			short int *num = (short int *)(data+256*inodelist[current_directory]->data_pntrs[j]+30);
			*num = currfileinode;
		}
		else
		{
			//printf("%d\n",inodelist[current_directory]->data_pntrs[j]+k*32);
			//printf("%s\n",dest);
			memcpy(data+256*inodelist[current_directory]->data_pntrs[j]+k*32,dirname,strlen(dirname)+1);
			short int *num = (short int *)(data+256*inodelist[current_directory]->data_pntrs[j]+k*32+30);
			*num = currfileinode;
		}
	}
	...
	inodelist[currfileinode]->lastaccessed = time(NULL);
	for(int i=0;i<8;i++)
		inodelist[currfileinode]->data_pntrs[i]=0;
	inodelist[currfileinode]->indirect_pntr = 0;
	inodelist[currfileinode]->dbl_ind_pntr = 0;
	inodelist[currfileinode]->parent_inode_no = current_directory;
	return 0;
}
...
extern int removedir(char* dirname,int dirinode)
{
	...
	i=0;
	while(inodelist[current_directory]->data_pntrs[i]!=0)
	{
		clearbit(super_block->db_bitmap,inodelist[current_directory]->data_pntrs[i]);
		super_block->dblocks_used--;
		i++;
		if(i==8)
			break;
...
extern int createfile(char* filename)
{
	...
	if(j!=8)
	{
		if(inodelist[current_directory]->data_pntrs[j]==0)
		{
			...
			inodelist[current_directory]->data_pntrs[j] = freedblock;
			memset(data+256*inodelist[current_directory]->data_pntrs[j],0,256);
			memcpy(data+256*inodelist[current_directory]->data_pntrs[j],filename,strlen(filename)+1);
			short int *num = (short int *)(data+256*inodelist[current_directory]->data_pntrs[j]+30);
			*num = currfileinode;
		}
		else
		{
			memcpy(data+256*inodelist[current_directory]->data_pntrs[j]+k*32,filename,strlen(filename)+1);
			short int *num = (short int *)(data+256*inodelist[current_directory]->data_pntrs[j]+k*32+30);
			*num = currfileinode;
		}
	}
...
int read_myfs (int fd, int nbytes, char *buff)
{
	...
	if(block_number < 8)
	{
		while(block_number<8)
		{
			while(block_offset<256)
			{
				...
				memcpy(buff+j, data+256*inodelist[i_num]->data_pntrs[block_number] + block_offset, 1);
				block_offset++;
				j++;
				filetable[fd].offset++;
			}
	...
	{
		b_num = (int*)(data+256*inodelist[i_num]->indirect_pntr);
	...
	{
		block_number-=72;
		sip = (int*)(data+256*inodelist[i_num]->dbl_ind_pntr);
...
extern int write_myfs(int fd, int nbytes, char* buff)
{
	...
	if(block_number < 8)
	{
		...
		{
			if(inodelist[i_num]->data_pntrs[block_number]==0)
			{
				int freedblock =getandsetfreeblock(super_block->db_bitmap);
				inodelist[file_inode]->data_pntrs[block_number]=freedblock;
			}
			for(; block_offset<256; block_offset++,j++,filetable[fd].offset++)
			{
				if(j==nbytes)
				...
				memcpy(data+256*inodelist[file_inode]->data_pntrs[block_number] + block_offset, buff+j, 1);
	...
	if(inodelist[i_num]->indirect_pntr==0)
	{
		int freedblock = getandsetfreeblock(super_block->db_bitmap);
		inodelist[file_inode]->indirect_pntr=freedblock;
	}
	block_offset = filetable[fd].offset%(256);
	if(block_number>=8 && block_number<72)
	{
		b_num = (int*)(data+256*inodelist[file_inode]->indirect_pntr);
		for(i=block_number-8;i<256/4;i++,block_number++)
		{
			if(*(b_num+i)==0)
			{
	...
	if(inodelist[i_num]->dbl_ind_pntr==0)
	{
		int freedblock =getandsetfreeblock(super_block->db_bitmap);
		inodelist[i_num]->dbl_ind_pntr=freedblock;
	}
